...
('python','Python','language'),('typescript','TypeScript','language'),('cpp','C++','language'),('sql','SQL','data'),('react','React','framework'),('nextjs','Next.js','framework'),('fastapi','FastAPI','framework'),('node','Node.js','framework'),('pytorch','PyTorch','ml'),('sklearn','scikit-learn','ml'),('transformers','Transformers','ml'),('mlops','MLOps','ml'),('docker','Docker','infra'),('kubernetes','Kubernetes','infra'),('aws','AWS','infra'),('cicd','CI/CD','infra'),('linux','Linux','systems'),('systemdesign','System design','systems'),('distributed','Distributed systems','systems'),('dsa','Data structures & algorithms','practice'),('rest','REST APIs','practice'),('git','Git','tooling'),('postgres','PostgreSQL','data'),('spark','Spark','data'),('embedded','Embedded systems','systems'),('cv','Computer vision','ml'),('ros','ROS','systems'),('figma','Figma','tooling'),('java','Java','language'),('javascript','JavaScript','language'),('go','Go','language'),('os','Operating Systems','systems'),('networks','Computer Networks','systems'),('dbms','DBMS','data'),('graphql','GraphQL','practice'),('express','Express','framework'),('springboot','Spring Boot','framework'),('django','Django','framework'),('flask','Flask','framework'),('angular','Angular','framework'),('azure','Azure','infra'),('terraform','Terraform','infra'),('mongodb','MongoDB','data'),('redis','Redis','data'),('kafka','Kafka','data'),('airflow','Airflow','data'),('pandas','Pandas','ml'),('numpy','NumPy','ml'),('tensorflow','TensorFlow','ml'),('nlp','NLP','ml'),('android','Android','framework')
...
-- The five quests above only covered Docker, System design, PyTorch and SQL.
-- A student whose top gaps lay elsewhere fell through to a tie-break, so the
-- "your biggest gap is X, here is a quest for X" chain quietly broke for them.
-- These cover the highest-impact gaps in the remaining role families.
('q_pandas','Build a data notebook that answers three questions','Load a real dataset with Pandas, clean it, and answer three questions with charts.','build','common','intro',70,4,array['ML Engineer','Data Analyst','Data Engineer'],'Pandas is among the most frequently required skills for your target roles.'),
('q_numpy','Implement gradient descent from scratch','Use NumPy to implement and visualise gradient descent on a small dataset.','learn','rare','intermediate',90,5,array['ML Engineer','Data Analyst'],'NumPy underpins the ML tooling your target roles expect.'),
('q_java','Ship a Spring Boot REST service','Build a small Java service with three endpoints, validation and tests.','build','epic','intermediate',130,8,array['Backend Engineer','Software Engineer'],'Java appears in a majority of historical backend postings.'),
('q_aws','Deploy a service to AWS with CI','Deploy a container to AWS and wire a CI pipeline that redeploys on push.','build','epic','advanced',150,9,array['DevOps Engineer','Backend Engineer'],'Cloud deployment is a core requirement for platform roles.'),
('q_js','Build an interactive dashboard in React','Create a responsive React dashboard that consumes a public API.','build','rare','intro',80,5,array['Frontend Engineer','Product Engineer','Software Engineer'],'JavaScript and React dominate frontend postings on campus.')
...
('q_pandas_s1','q_pandas','Load and clean the dataset',0),('q_pandas_s2','q_pandas','Answer three questions with charts',1),
('q_numpy_s1','q_numpy','Implement the gradient step',0),('q_numpy_s2','q_numpy','Plot the loss curve',1),
('q_java_s1','q_java','Scaffold the Spring Boot service',0),('q_java_s2','q_java','Add validation and tests',1),('q_java_s3','q_java','Document the endpoints',2),
('q_aws_s1','q_aws','Containerise and push the image',0),('q_aws_s2','q_aws','Deploy to AWS',1),('q_aws_s3','q_aws','Wire CI to redeploy on push',2),
('q_js_s1','q_js','Build the component layout',0),('q_js_s2','q_js','Wire it to a public API',1),
...
insert into public.quest_skills(quest_id,skill_id) values ('q_docker','docker'),('q_sysdesign','systemdesign'),('q_oss','pytorch'),('q_oss','git'),('q_sql','sql'),('q_sql','postgres'),('q_pandas','pandas'),('q_numpy','numpy'),('q_java','java'),('q_aws','aws'),('q_js','javascript'),('q_js','react') on conflict do nothing;
