...
on conflict (id) do update set name=excluded.name, branch=excluded.branch, academic_year=excluded.academic_year, goal_role=excluded.goal_role, interests=excluded.interests, wants_to_learn=excluded.wants_to_learn, collaboration_intent=excluded.collaboration_intent, looking_for_team=excluded.looking_for_team;
...
select id,'profile',id::text,'local-deterministic-v1',lower(name||' '||goal_role||' '||array_to_string(interests,' ')||' '||array_to_string(wants_to_learn,' ')),encode(digest(lower(name||' '||goal_role||' '||array_to_string(interests,' ')||' '||array_to_string(wants_to_learn,' ')),'sha256'),'hex'),array_fill(0::real,array[1024])::vector from public.profiles
